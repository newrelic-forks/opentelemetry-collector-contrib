...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerQuerySampleQueryPlanEvent(ctx, timestamp, AttributeDbSystemNameMysql, "mysql.query_plan.hash-val", "db.namespace-val", "mysql.query_plan-val")
			...
			allEventsCount++
			lb.RecordDbServerTopQueryQueryPlanEvent(ctx, timestamp, AttributeDbSystemNameMysql, "mysql.query_plan.hash-val", "db.namespace-val", "mysql.query_plan-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample.query_plan":
					assert.False(t, validatedEvents["db.server.query_sample.query_plan"], "Found a duplicate in the events slice: db.server.query_sample.query_plan")
					validatedEvents["db.server.query_sample.query_plan"] = true
					lr := lrs.At(i)
					assert.Equal(t, timestamp, lr.Timestamp())
					assert.Equal(t, pcommon.TraceID(traceID), lr.TraceID())
					assert.Equal(t, pcommon.SpanID(spanID), lr.SpanID())
				...
				case "db.server.top_query.query_plan":
					assert.False(t, validatedEvents["db.server.top_query.query_plan"], "Found a duplicate in the events slice: db.server.top_query.query_plan")
					validatedEvents["db.server.top_query.query_plan"] = true
					lr := lrs.At(i)
					assert.Equal(t, timestamp, lr.Timestamp())
					assert.Equal(t, pcommon.TraceID(traceID), lr.TraceID())
					assert.Equal(t, pcommon.SpanID(spanID), lr.SpanID())

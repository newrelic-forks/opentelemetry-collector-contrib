...
| mysql.wait_event_type | Top-level wait classification derived from the second path segment of the performance_schema wait event name ('io', 'lock', 'synch'), or 'CPU'/'User sleep'/'other' when not actually waiting. MySQL's analogue of Oracle's WAIT_CLASS / SQL Server's wait_category. Paired with mysql.wait_event. | Any Str | - |
| mysql.wait_event | Detailed wait breakdown -- the performance_schema wait event name with its category segment stripped (e.g. 'table/sql/handler', 'mutex/innodb/checkpoint_state'), or 'CPU'/'User sleep'/'other' when not actually waiting. MySQL's analogue of Oracle's EVENT / SQL Server's wait_type. Paired with mysql.wait_event_type. Distinct from mysql.wait_type, which reports the raw, unprocessed event name and is unchanged from upstream. | Any Str | - |
...
| mysql.blocking.blocker.thread_id | The thread_id of the session currently holding the lock this session is waiting on, sourced from performance_schema.data_lock_waits. 0 indicates the session is not currently blocked. Reflects the oldest of multiple concurrent blockers, not an arbitrary one — see mysql.blocking.blockers for the complete list. Derived from element [0] of mysql.blocking.blockers; kept as its own scalar for the same NRQL-cannot-parse-JSON reason documented on mysql.blocking.blocker.session_id. | Any Int | - |
...
| mysql.blocking.blocker.session_id | The PROCESSLIST_ID (session id) of the session currently holding the lock this session is waiting on, resolved from BLOCKING_THREAD_ID via performance_schema.threads. 0 indicates the session is not currently blocked, or that the blocker could not be resolved (see mysql.blocking.blockers for the unresolved-vs-unblocked distinction). Reflects the oldest of multiple concurrent blockers, not an arbitrary one. Derived from element [0] of mysql.blocking.blockers, but kept as its own scalar attribute (not removed as "redundant") because NRQL cannot index into or parse a JSON string attribute — a numeric mysql.session.id cross-reference join, FACET, or alert threshold needs an actual int, which the JSON array cannot provide on its own. Use this, not mysql.blocking.blocker.thread_id, to cross-reference the blocking session against mysql.session.id elsewhere on the dashboard. | Any Int | - |
...
| mysql.blocking.blockers | A JSON array of {thread_id, session_id} tuples, one per concurrent InnoDB row-lock blocker for this session, ordered best-effort by blocking-transaction start time (oldest/most-likely-root-cause first). session_id is null when that blocker's PROCESSLIST_ID could not be resolved (e.g. it disconnected between reads). "[]" when not blocked. mysql.blocking.blocker.thread_id/.session_id/.count are derived from this array (element [0] and array length respectively) but are not redundant with it — NRQL cannot parse or index into a JSON string attribute, so the scalars are the only way to facet, filter, alert on, or cross-reference this data numerically. Use this attribute when you need the complete blocker list; use the scalars for everything else. | Any Str | - |
| mysql.blocking.blocker.count | The number of concurrent InnoDB row-lock blockers currently holding a lock this session is waiting on. 0 indicates the session is not currently blocked. Derived from mysql.blocking.blockers (its array length) and kept as a separate scalar attribute because NRQL has no native JSON array-length function — this is the only way to facet, filter, or alert on blocker count directly. | Any Int | - |

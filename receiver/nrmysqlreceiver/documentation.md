...
| mysql.blocking.blockers | A JSON array of {thread_id, session_id} tuples, one per concurrent InnoDB row-lock blocker for this session, ordered oldest blocking-transaction first (element [0] is the most likely root cause). This ordering is enforced by the receiver itself, not by SQL — MySQL's JSON_ARRAYAGG element order is unspecified and was confirmed live to not reliably reflect the query's ORDER BY. session_id is null when that blocker's PROCESSLIST_ID could not be resolved (e.g. it disconnected between reads); a blocker whose transaction age couldn't be resolved sorts last. "[]" when not blocked. This is the only source for individual blocker identity (thread_id/session_id) — the receiver does not also expose those as separate scalar attributes, since doing so would only serve per-row display, which a dashboard/frontend can already get by parsing this JSON directly; use mysql.blocking.blocker.count (not a value from this array) for any NRQL-side filter, facet, or alert threshold. | Any Str | - |
| mysql.blocking.blocker.count | The number of concurrent InnoDB row-lock blockers currently holding a lock this session is waiting on. 0 indicates the session is not currently blocked — use "mysql.blocking.blocker.count > 0" as the "is this session blocked" filter/gate (replaces the retired mysql.blocking.blocker.thread_id > 0 pattern). Derived from mysql.blocking.blockers (its array length) and kept as a separate scalar attribute because NRQL has no native JSON array-length function — this is the only way to facet, filter, or alert on blocker count directly. | Any Int | - |

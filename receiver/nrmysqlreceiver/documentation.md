...
| mysql.blocking.blockers | A JSON array of {thread_id, session_id} tuples, one per concurrent InnoDB row-lock blocker for this session, ordered best-effort by blocking-transaction start time (oldest/most-likely-root-cause first). session_id is null when that blocker's PROCESSLIST_ID could not be resolved (e.g. it disconnected between reads). "[]" when not blocked. mysql.blocking.blocker.thread_id/.session_id are derived from element [0] of this array. | Any Str | - |
| mysql.blocking.blocker.count | The number of concurrent InnoDB row-lock blockers currently holding a lock this session is waiting on. 0 indicates the session is not currently blocked. Derived from mysql.blocking.blockers. | Any Int | - |

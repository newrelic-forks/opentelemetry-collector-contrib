...
func TestDetectInstanceInfo_VersionQueryFails(t *testing.T) {
	...
	assert.Equal(t, 1, logs.FilterMessage("oracledbreceiver: failed to detect Oracle version; oracle.db.pdb attribute will not be set").Len())
}
...
func TestSetupResourceBuilder_NoPDB(t *testing.T) {
	// When not connected to a PDB, oracle.db.pdb must not appear in resource.
...
func TestSetupResourceBuilder_WithPDB(t *testing.T) {
	// When connected to a PDB, oracle.db.pdb must be set on the resource.
	cfg := metadata.NewDefaultMetricsBuilderConfig()
	scrpr := oracleScraper{
		mb:                   metadata.NewMetricsBuilder(cfg, receivertest.NewNopSettings(metadata.Type)),
		metricsBuilderConfig: cfg,
	...
	require.True(t, ok, "oracle.db.pdb should be set when connected to a PDB")

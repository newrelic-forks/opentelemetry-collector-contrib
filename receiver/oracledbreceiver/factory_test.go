...
func TestResolveServerEndpointFromDataSource(t *testing.T) {
	...
	tests := []struct {
		...
		// url.Parse reads neither form, so the host stays undetermined. The connection may still
		// succeed, so it must not be reported as the collector's own hostname.

...
func TestConcurrentMetricsScraper_PartialFailuresProduceJoinedErrors(t *testing.T) {
	...
	// Only b's metrics should land in the merged result.
...
func TestConcurrentMetricsScraper_RunsChildrenInParallel(t *testing.T) {
	...
	// Sequential would take ~numChildren*delay. Allow generous slack for CI.
...
func TestConcurrentMetricsScraper_ZeroCapIsClampedToOne(t *testing.T) {
	// cap=0 must not deadlock — it should be clamped to 1 and still run all children.
	const n = 3
	live := &atomic.Int32{}
	peak := &atomic.Int32{}
...
// Sanity check that the production wiring (typed *sqlServerScraperHelper slice)
// compiles via the wrapper constructor. We don't invoke it — just ensure the
...
func TestNewConcurrentMetricsScraper_AcceptsHelperSlice(t *testing.T) {
	...
	// Use sync to silence "imported and not used" if no other path references it.
	_ = sync.Mutex{}
}

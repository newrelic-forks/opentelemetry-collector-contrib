...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerQuerySampleEvent(ctx, timestamp, "client.address-val", 11, "db.namespace-val", "db.query.text-val", "db.system.name-val", "network.peer.address-val", 17, 29, "sqlserver.blocking.start_time-val", "sqlserver.context_info-val", "sqlserver.command-val", 18.100000, 27, 35.100000, 22.100000, 23, 32, 26.100000, "sqlserver.query_hash-val", "sqlserver.query_plan_hash-val", "sqlserver.query_start-val", 15, "sqlserver.request_status-val", "sqlserver.wait.resource.id-val", "sqlserver.wait.resource.type-val", 19, 36.100000, 33, "sqlserver.session.program_name-val", "sqlserver.session.started-val", 20, "sqlserver.session_status-val", 28.100000, 24, 37, "sqlserver.wait_resource-val", 19.100000, "sqlserver.wait_type-val", 16, "user.name-val", "sqlserver.procedure_id-val", "sqlserver.procedure_name-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("sqlserver.session.total_elapsed_time")
					assert.True(t, ok)
					assert.Equal(t, 36.100000, attrVal.Double())
					attrVal, ok = lr.Attributes().Get("sqlserver.session.host_process_id")
					assert.True(t, ok)
					assert.EqualValues(t, 33, attrVal.Int())
					attrVal, ok = lr.Attributes().Get("sqlserver.session.program_name")
					assert.True(t, ok)
					assert.Equal(t, "sqlserver.session.program_name-val", attrVal.Str())

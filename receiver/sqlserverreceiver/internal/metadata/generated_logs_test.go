		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.top_query":
					...
					assert.True(t, ok)
					assert.Equal(t, "sqlserver.procedure_name-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("sqlserver.last_execution_time")
					assert.True(t, ok)
					assert.Equal(t, "sqlserver.last_execution_time-val", attrVal.Str())
				}
			}
		})
	}

...
func newPostgreSQLScraper(
	...
	separateSchemaAttr := metadata.ReceiverNrpostgresqlSeparateSchemaAttrFeatureGate.IsEnabled()
	useOTelSemconv := metadata.ReceiverNrpostgresqlUseOTelSemconvFeatureGate.IsEnabled()
	...
	if separateSchemaAttr && useOTelSemconv {
		return nil, fmt.Errorf("feature gates %s and %s are mutually exclusive and cannot both be enabled",
			metadata.ReceiverNrpostgresqlSeparateSchemaAttrFeatureGate.ID(),
			metadata.ReceiverNrpostgresqlUseOTelSemconvFeatureGate.ID())
	}

	if !separateSchemaAttr && !useOTelSemconv {
		settings.Logger.Warn(
			fmt.Sprintf("Feature gate %s is not enabled. Please see the README for more information: %s", metadata.ReceiverNrpostgresqlSeparateSchemaAttrFeatureGate.ID(), readmeURL),
		)
	}
...
func (p *postgreSQLScraper) collectQuerySamples(ctx context.Context, dbClient client, limit int64, mux *errsMux, logger *zap.Logger) {
	...
	attributes, newestQueryTimestamp, err := dbClient.getQuerySamples(ctx, limit, p.newestQueryTimestamp, p.excludedDatabases, p.config.QuerySampleCollection.AllowedCommentKeys, logger)
...
func (p *postgreSQLScraper) probeExplainFunctionIfNeeded(ctx context.Context, database string, dbClient client) {
	if p.config.TopQueryCollection.ExplainFunctionName == "" {
		return
	}
	...
	quoted := quoteExplainFunctionName(p.config.TopQueryCollection.ExplainFunctionName)
	...
	if pqErr, ok := errors.AsType[*pq.Error](err); ok {
		if pqErr.Code == pqerror.UndefinedFunction {
			p.logger.Warn("EXPLAIN helper function not found, falling back to inline EXPLAIN",
				...
				zap.String("explain_function_name", p.config.TopQueryCollection.ExplainFunctionName),
				zap.Error(err))
		} else {
		...
		if !ok && explained < maxExplainEachInterval {
			...
			if err == nil {
				...
				if state, cached := p.explainFunctionCache.Get(database); cached && state.available {
					explainFunction = quoteExplainFunctionName(p.config.TopQueryCollection.ExplainFunctionName)
				}

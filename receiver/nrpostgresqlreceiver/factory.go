...
func createLogsReceiver(
	...
	if cfg.LogsBuilderConfig.Events.DbServerTopQuery.Enabled {
		...
		ns, err := newPostgreSQLScraper(params, cfg, clientFactory, newCache(int(cfg.TopQueryCollection.TopNQuery*10*2)), newTTLCache[string](cfg.TopQueryCollection.QueryPlanCacheSize, cfg.TopQueryCollection.QueryPlanCacheTTL), newTTLCache[explainSetupState](cfg.TopQueryCollection.QueryPlanCacheSize, cfg.TopQueryCollection.ExplainFunctionCacheTTL))
		if err != nil {
			return nil, err
		}
		s, err := scraper.NewLogs(func(ctx context.Context) (plog.Logs, error) {

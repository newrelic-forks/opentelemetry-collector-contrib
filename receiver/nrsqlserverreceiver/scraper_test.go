...
func TestSuccessfulScrape(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			testutil.SetFeatureGateForTest(t, metadata.ReceiverNrsqlserverRemoveServerResourceAttributeFeatureGate, test.removeServerResourceAttributeFeatureGate)
			cfg := createDefaultConfig().(*Config)
			cfg.Username = "sa"
			cfg.Password = "password"
			cfg.Port = 1433
...
func (mc mockClient) QueryRows(context.Context, ...any) ([]sqlquery.StringMap, error) {
	...
	case getSQLServerTopProcedureQuery(mc.instanceName):
		queryResults, err = readFile("topProcedureQueryData.txt")
	default:
...
// event enabled, backed by testdata/topProcedureQueryData.txt.
